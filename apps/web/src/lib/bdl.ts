/**
 * BallDontLie API client for live NBA markets on Vercel.
 *
 * Ported from packages/services/src/catalog/bdl.ts for serverless use.
 * Requires BDL_API_KEY env var. Gracefully returns [] if not set.
 */
import type { Market, Leg } from "@parlaycity/shared";

const BDL_BASE = "https://api.balldontlie.io/v1";
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
export const NBA_LEG_ID_OFFSET = 1000;
...
function estimateMoneylinePPM(
  ...
  const homeExpected = (homeAvgPts + awayAvgOppPts) / 2 + 3;
...
async function fetchTeamStats(teamIds: number[]): Promise<Map<number, { avgPts: number; avgOppPts: number }>> {
  ...
  try {
    ...
    url.searchParams.append("type", "base");
...
function gameToMarket(
  game: BDLGame,
  legIdBase: number,
  teamStats: Map<number, { avgPts: number; avgOppPts: number }>,
): Market {
  ...
  const earliestResolve = cutoffTime + 4 * 3600;
